defmodule TranshookWeb.HookLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all hooks", %{conn: conn, hook: hook} do
      {:ok, _index_live, html} = live(conn, Routes.hook_index_path(conn, :index))
    ...
    test "saves new hook", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, Routes.hook_index_path(conn, :index))

      assert index_live |> element("a", "New Hook") |> render_click() =~
               "New Hook"

      assert_patch(index_live, Routes.hook_index_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.hook_index_path(conn, :index))
    ...
    test "updates hook in listing", %{conn: conn, hook: hook} do
      ...
      assert_patch(index_live, Routes.hook_index_path(conn, :edit, hook))
  ...
  describe "Show" do
    ...
    test "displays hook", %{conn: conn, hook: hook} do
      {:ok, _show_live, html} = live(conn, Routes.hook_show_path(conn, :show, hook))
    ...
    test "updates hook within modal", %{conn: conn, hook: hook} do
      {:ok, show_live, _html} = live(conn, Routes.hook_show_path(conn, :show, hook))

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Hook"

      assert_patch(show_live, Routes.hook_show_path(conn, :edit, hook))
      ...
      {:ok, _, html} =
        ...
        |> form("#hook-form", hook: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.hook_show_path(conn, :show, hook))
